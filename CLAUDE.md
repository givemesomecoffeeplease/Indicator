...
### 수정
`updateEditCursor()` 맨 앞에 `clampCursor()` 호출 추가 — 이제 두 함수가 항상 같은 보정된 `cursorB`/`cursorK`를 사용하므로 호출 순서와 무관하게 항상 일치. 실제로 박 수가 다른 마디로 전환하는 시나리오를 라이브로 재현해 `cursorB`가 자동 보정되고 커서 x좌표가 새 마디 범위 안으로 들어오는 것까지 확인 완료.
