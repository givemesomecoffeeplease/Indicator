...
### 진행 상황 (2026-07-16)
- **1~6단계 구현 완료, 빌드 성공, /Applications에 설치됨** (실사용 검증 전 상태 — 문제가 생기면 `git checkout pre-midi-slide` 후 재빌드).
- 구현 내용: 계획대로 전부. 세부 구현 메모:
  - 편집 상태 모델: 기존 `{splits,segData}` → `{segData:[슬라이드]}` 로 단순화. splits/getSegs/linked(실시간 미러) 개념 제거.
  - 찍기 커서(`tapCtx`): "이미 확정된 경계 중 현재 위치보다 앞의 것들 다음"이 다음 찍힐 경계. 섹션 진입·뒤로 점프(1.5초 이상) 시 재계산 → 처음부터 다시 들으면 1번 경계부터 재찍기 가능.
  - 드래그: 드래그 중엔 DOM 스타일만 갱신(재렌더 시 pointer capture 유실 방지), 놓을 때 setState.
  - 재생헤드: SSE `sectionElapsedSec` + 수신 후 로컬 경과 시간으로 클라이언트 보간 (`liveElapsed`).
  - `/save`는 모든 섹션을 loadState 기준으로 전송 (dirty 우선/원본 폴백) — 기존 dirty-only 방식보다 단순.
  - 검증 완료: 편집·싱어·밴드 JS 문법(node --check), 슬라이드 추가/타임라인/임시(guess) 핸들 렌더링, 저장 왕복(startSec 7.77·null·슬라이드 노트 Swift Codable 보존), 싱어 뷰 초 기반 슬라이드 선택+슬라이드 노트 표시.
- **다음 세션에서 할 일**: Logic 실제 재생으로 라이브 검증 — ① 재생헤드/현재 슬라이드 하이라이트 ② 전환 찍기(스페이스바) ③ 핸들 드래그 ④ 따라가기 곡 자동 전환 ⑤ 뷰어 실시간 전환 확인 → 이상 없으면 커밋된 상태 그대로 릴리즈(gh release) 여부 사용자에게 확인.
- 참고: LyricsStore는 메모리 전용이라 앱 재시작 시 가사 비어 있음 — 검증 시 HTML 가져오기로 데이터 로드 후 테스트.
