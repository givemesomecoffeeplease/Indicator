...
- **다음 세션에서 할 일**: Logic 실제 재생으로 라이브 검증 — ① 재생헤드가 곡 타임라인을 따라 끊김 없이 흐르는지 ② 마커를 넘을 때 섹션 첫 슬라이드가 자동 하이라이트되는지 ③ Enter로 넘김 기록 ④ 핸들 드래그(특히 섹션 첫 슬라이드 핸들을 마커 위로 끌어 음수 오프셋 만들기) ⑤ 따라가기 곡 자동 전환 ⑥ 뷰어 실시간 전환 확인 → 이상 없으면 릴리즈(gh release) 여부 사용자에게 확인.
