...
## 2026-09-13 (2차) — 변박 구간 마디 위치 계산 버그 수정 + `/chart` 첫 화면을 리얼드럼으로 변경

- **변박(예: 3/4→4/4) 구간에서 채보 하이라이트가 재생 위치를 못 쫓아오는 버그 수정**: `ScheduleStore.barPositionAt(mtcSeconds:)`이 마지막 템포 마커 이후 경과 시간을 마디 수로 환산할 때, **그 경과 구간 안에 박자표가 바뀌는 지점이 있어도 "지금 시점(mtcSeconds)의 박자표" 하나만으로 전체 구간을 계산**하고 있었음 — 예를 들어 3/4 구간을 지나 4/4로 넘어간 직후엔, 3/4로 흘러간 구간까지도 전부 4/4 기준 분모로 계산해버려 마디 수가 어긋남. 구간 안의 박자표 변경 지점들을 `boundaries`로 뽑아 그 지점마다 나눠, 각 구간을 그 구간의 실제 박자표로 따로 누적하도록 수정 — 변박이 여러 번 있어도 각 구간이 자기 박자표로 정확히 계산됨. 이 함수는 `/chart`(리얼드럼 라이브 하이라이트)뿐 아니라 `StateEngine`의 `barPosition` 계산에도 쓰이므로, `/drum`(라이브 채보 뷰어)의 변박 추적도 같이 정확해짐.
- **`/chart` 첫 화면을 그리드 → 리얼드럼으로 변경**: 초기화 마지막에 `setMode('drum')` 한 줄 추가 — 탭 선택 표시·화면 전환·드럼킷 빌드까지 기존 `setMode()` 함수가 그대로 처리하므로 다른 정적 기본값(HTML의 `display:none` 등)은 안 건드림.
- 로컬에서 `/chart` 첫 로드 시 리얼드럼 탭이 선택되고 화면도 바로 뜨는 것 확인.

---

## 2026-09-13 — `/edit` 가져오기를 병합→완전 교체 방식으로 변경

- **배경**: "전체 가져오기"(또는 곡별 가져오기)가 지금까지 섹션 단위 **병합** 방식이었음 — 가져온 파일 안의 섹션과 현재 세트리스트 섹션을 `sec+occIdx`로 매칭해서 매칭된 섹션만 덮어쓰고, 파일에 없는 섹션은 기존 값을 그대로 남겨뒀음. 사용자가 이 방식이 "옛 섹션이 유령처럼 남을 수 있다"는 문제를 인지하고 완전 교체 방식으로 바꿔달라고 요청.
- **수정** (`WebServer.swift` `handleImportFile`, `/edit` 내장 스크립트): 새 `clearSec(destSec)` 헬퍼 추가 — `slides=[]`, `sessionNote`/`singerNote`를 빈 문자열로 초기화. 곡별 가져오기·전체 가져오기 두 경로 모두, 가져온 파일의 섹션과 매칭된 섹션은 `mergeSec`으로 덮어쓰고(기존과 동일), **매칭 안 된 나머지 섹션은 전부 `clearSec`으로 초기화**하도록 변경. `mergeSec`도 노트 필드의 "값 없으면 기존 값 유지" 폴백을 제거해 완전 교체가 되도록 정리.
- **곡/드럼채보는 기존에도 이미 완전 교체였음** (참고, 이번에 안 건드림): 전체 가져오기에서 파일에 없는 곡은 아예 손대지 않음(그대로 유지) — 이건 의도된 동작으로 유지. 드럼 채보는 `DrumChartStore`가 곡명 키로 통째로 저장하는 구조라 업로드하면 이미 완전 교체됨. 단, 가져온 파일에 드럼 채보가 아예 없는 경우 기존 채보를 지우지는 않음(삭제 API가 없어 이번 범위에서는 그대로 둠) — 필요해지면 별도로 다룰 것.
- 로컬 `/edit`에서 실제 곡("성령의 바람", 섹션 18개) 대상으로 `handleImportFile`을 직접 호출해 검증: 가져온 파일에 있던 Intro는 새 가사로 교체되고, 파일에 없던 Verse(기존에 테스트 가사를 넣어둠)는 `slides=[]`로 완전히 비워지는 것 확인.

---

## 2026-09-09 — `/audience` 전체화면 버튼 제거(화면 아무 데나 첫 클릭으로 대체)

- 오른쪽 아래 옅은 전체화면 버튼도 화면에 남는 요소라 요청으로 완전히 제거. 대신 **화면 아무 데나 처음 한 번 클릭**하면 전체화면에 들어가도록 변경(`enterFullscreenOnce`) — 브라우저가 진짜 사용자 동작 없이는 전체화면을 절대 허용 안 해서 버튼 없이 들어가려면 이 방법뿐. 한 번 발동하면 리스너가 스스로 떨어져 나가 그 뒤로는 화면을 눌러도 아무 반응 없음.
- 전체화면 빠져나오는 길은 브라우저 기본 **Esc뿐**(요청대로) — 애초에 앱 쪽에서 Esc를 대체/차단할 방법도 없고 막을 이유도 없음.
- 로컬에서 클릭 리스너가 정확히 한 번만 발동하고 스스로 제거되는 것 확인.

---

## 2026-09-08 (2차) — `/audience` 청중용 가사 전체화면 뷰어 신설

- **배경**: 청중이 직접 보는 화면은 검정 배경에 흰 가사만 있어야 하고 그 외엔 아무 것도(섹션명·코드·다음 가사·헤더 등) 없어야 한다는 요청 — 전체화면 지원, 슬라이드 전환 타이밍은 기존 라이브 동기화를 그대로 따름.
- **신규 파일** `Resources/audience.html`, 라우트 `/audience`(`WebServer.swift`에 `handleAudience` 추가, 랜딩 페이지에 "청중 화면" 링크도 추가). Xcode 프로젝트가 파일을 수동 등록하는 방식이라 `project.pbxproj`에 `PBXBuildFile`/`PBXFileReference`/그룹/Resources 빌드 페이즈 4곳에 직접 추가(다른 리소스 파일들과 동일한 패턴).
- **타이밍은 완전히 재사용**: `/band`(index.html)의 `resolveSlideSecs`·`fetchLyricCache`·`findCurrentEntry` 로직을 그대로 복사 — 세 화면(운영자·싱어·청중)이 서로 다른 순간에 슬라이드가 넘어가면 안 되므로 같은 데이터·같은 계산을 그대로 씀.
- **표시는 완전히 새로 단순화**: 토큰에서 `type==='char'`만 뽑아 순수 텍스트 줄로 만드는 `plainLines()` 신설(코드·고스트 자리 전부 버림 — 청중 화면엔 필요 없음). 간주 슬라이드(`isInstrumental`)는 자연히 빈 화면(의도한 동작). 화면 폭에 안 맞으면 공통 비율로 축소하는 `fitLyric()`은 index.html의 `fitLyricLines`와 같은 원리로 새로 작성(다음 가사·코드가 없어서 구조가 더 단순함).
- **전체화면**: 오른쪽 아래 아주 옅은 버튼 하나만 예외로 둠 — 누르면 `requestFullscreen()`, 전체화면에 들어가는 순간(`fullscreenchange`) 그 버튼마저 `display:none`으로 완전히 사라져서 화면엔 정말 가사밖에 안 남음.
- 다른 뷰어와 동일하게 SSE "조용함 감시" 재연결(2026-09-08 1차)도 처음부터 같이 넣음(오래 켜둘 화면이라 특히 중요).
- 로컬에서 실제로 `renderLyric()`을 직접 호출해 검정 배경+흰 가사만 뜨는 것, 긴 줄이 폭 안에 맞게 자동으로 줄어드는 것까지 스크린샷으로 확인.

---

## 2026-09-08 — 모든 뷰어에 SSE "조용함 감시" 재연결 추가 (오래 두면 멈춰있던 버그)

- **증상 리포트**: 뷰어를 오래 재생 없이 켜뒀다가 다시 재생하면 화면이 멈춰있고, 새로고침해야 다시 움직임.
- **원인**: `/band`·`/singer`·`/drum`·`/chart`(로직 따라가기) 전부 이미 "연결 끊기면(`onerror`) 재연결"은 있었는데, Mac 잠듦·네트워크 순간 끊김 등으로 연결이 **겉보기엔 열려있지만 실제로는 죽은** 상태가 되면 `onerror`가 안 뜨는 경우가 있음(잘 알려진 EventSource 함정). 게다가 로직이 정지해 화면이 안 바뀌는 동안은 서버가 원래 아무 신호도 안 보내서, 그 조용한 구간 중 연결이 죽어도 양쪽 다 알아챌 방법이 없었음.
- **처음 제안했던 "1분마다 무조건 재연결"은 사용자가 "곡 중간에 끊기지 않냐"고 지적해 기각** — 대신 **"오래 조용하면(90초 이상 아무 신호 없음) 그때만 재연결"** 방식으로 확정. 재생 중엔 상태가 초당 최대 20번씩 바뀌며 계속 신호가 오므로 이 타이머가 절대 안 끝나 곡 중간엔 전혀 안 끼어들고, 로직이 정지해 정말 아무 신호도 없는 조용한 구간(마침 화면도 안 바뀌는 중)에서만 조용히 재연결됨 — 사용자 확인 후 이 방식으로 4개 파일(`index.html`, `singer.html`, `drum.html`, `chart.html`) 전부에 동일하게 적용.
- 로컬에서 `lastMsgAt`을 인위적으로 90초 전으로 돌려 10초 뒤 자동 재연결(`es` 인스턴스 교체) 확인, 반대로 방금 신호 받은 것처럼 두면 재연결이 전혀 안 일어나는 것도 확인.

---

## 2026-09-06 (16차) — `/drum` 섹션명·가사 캡션을 마디 아래→위로 이동 + v0.9.1 릴리즈

- `/drum`에서 각 줄의 섹션명·가사 캡션이 그 줄 악보 **아래**에 있었는데(DOM상 `<svg>` 다음에 `.cap`), 사용자 확인 후 **위로** 이동. `appendSongBlock`의 줄 템플릿에서 `.cap`과 `<svg>` 순서를 바꾸고, 간격용 margin도 서로 자리를 바꿔 붙임(`.cap`이 margin-bottom, `svg`가 margin-top) — 시각적 간격 자체는 그대로 유지.
- 로컬에서 실제 줄 DOM의 자식 순서(`DIV`→`svg`)와 스크린샷으로 위치 확인.
- Universal 릴리즈 v0.9.1로 재생성.

---

## 2026-09-06 (15차) — `/chart` 그리드·리얼드럼 탭 순서 변경 + Universal 릴리즈 교체

- `chart.html` 상단 탭 순서를 "그리드 → 리얼드럼"에서 "**리얼드럼 → 그리드**"로 변경(마크업 순서만 바꿈, 순서 의존 CSS 없는 것 확인 후 진행). 기본 선택(`class="sel"`)은 그대로 그리드 유지.
- 이번 세션(9/6) 전체 작업분(플램 기능·32비트 그리드·리얼드럼 단축키 커스터마이즈·BPM 직접입력·녹음 버튼·메트로놈 8비트·`/edit`↔`/chart` 곡 연결·`/drum` 실시간 반영 등)을 담아 Universal(Intel+Apple Silicon) 릴리즈 빌드 후 GitHub 릴리즈로 교체.

---

## 2026-09-06 (14차) — `/drum`에 플램이 안 보이던 버그 수정 + 저장 즉시 실시간 반영

- **플램이 `/drum`에서 아예 안 보이던 버그 수정**: 플램(2026-09-06 8차)을 만들 때 `/chart` 편집 화면의 자체 렌더링 엔진에만 그리기 코드를 넣고, `/drum`이 쓰는 공유 렌더링 엔진(`notation.js`)에는 안 넣었음 — 이 둘은 "겉보기 결과가 항상 같아야 한다"는 원칙(코드 주석에 명시)이 있는데 이번에 놓쳤던 것. 사용자가 `/chart`에서 플램 넣고 저장했는데 `/drum`엔 안 보인다고 리포트해서 발견. `notation.js`에 `drawFlam()`과 그리기 호출을 chart.html과 완전히 동일하게 이식.
- **`/save-drum-chart` 저장이 이미 열려있는 `/drum` 화면에 실시간 반영되도록 신규**: 예전엔 저장만 하고 아무 신호가 없어서, 공연 중 `/chart`로 급히 고쳐도 이미 켜진 `/drum`은 새로고침 전까지 옛 악보 그대로였음. 가사가 이미 쓰는 `lyrics-updated`와 같은 SSE 채널(`/events`)에 `drum-updated`(곡 이름 포함) 이벤트를 새로 추가 — 기존 연결에 이벤트 하나 더 얹는 것뿐이라 부하는 무시할 수준(사용자가 "무거워지지 않는다면"이라고 확인 요청해서 이 점 명시).
  - `/drum`은 지금 보고 있는 곡이면 `startFreshWith()`로 안전하게 다시 그리고(화면 순서가 안 꼬이게), 미리 불러와둔 "다음 곡"이면 그 자리만 조용히 교체. 화면에 없는 곡이면 아무 것도 안 함(그 곡 차례가 오면 어차피 새로 받아옴).
- 로컬에서 실제 `/save-drum-chart` 호출 → `/drum` 탭이 새로고침 없이 자동으로 플램까지 정확히 반영하는 것을 실제 SSE 왕복으로 확인(수동으로 이벤트를 흉내낸 게 아니라 진짜 저장 API를 호출해서 그 결과로 확인).

---

## 2026-09-06 (13차) — `/edit` 드래그앤드롭이 드럼 채보(.json)도 자동 구분해서 받도록 확장

- **배경**: `/edit`에 이미 `.html`(가사 백업) 파일을 곡 화면에 끌어놓으면 자동으로 가져오는 드래그앤드롭이 있었는데, 드럼 채보(`.drum.json`)는 이 경로로 못 들어오고 반드시 "드럼 채보 업로드" 버튼(파일 선택창)을 따로 눌러야 했음 — 가사·드럼 둘 다 같은 자리에 그냥 끌어놓기만 하면 앱이 확장자로 알아서 구분해달라는 요청.
- **수정** (`WebServer.swift` `/edit` 임베디드 JS): 기존 `drop` 핸들러가 `.html` 파일만 찾아서 없으면 바로 에러 메시지였던 것을, `.html`이 있으면 기존대로 가사 가져오기, 없고 `.json`이 있으면 드럼 채보 저장 경로로 가도록 확장. 드럼 쪽 검증·저장은 "드럼 채보 업로드" 버튼이 쓰던 것과 **완전히 동일한 코드**를 그대로 재사용(측정 형식 확인 → `/save-drum-chart` POST) — 로직이 두 벌로 나뉘어 나중에 하나만 고치는 실수를 막음. 저장 성공하면 `selectSong(song)`을 다시 불러 "드럼 채보: 있음" 표시가 바로 갱신되게 함.
- **디스크 저장(서버 재시작 시 드럼 채보 유지) 여부는 보류하기로 확정**: 사용자가 직접 관리하는 파일 폴더가 "진짜 원본"이고 서버(`DrumChartStore`)는 "지금 라이브 뷰어가 보는 캐시"일 뿐이라는 역할 분담을 그대로 유지하기로 함 — 두 군데에 저장하면 "어느 게 최신인지" 비교할 방법이 없어 오히려 혼란만 커진다는 게 이유. 대신 "`/chart`에서 고치고 → 서버에 저장 → 바로 다운로드해서 폴더에도 반영"하는 기존 습관을 계속 쓰기로 함(2026-09-06, `/edit`↔`/chart` 곡 연결 기능을 만들며 논의).
- 로컬에서 실제로 `/save-drum-chart`에 저장 후 `/api/drumChart`로 재조회해 측정 데이터가 정확히 왕복되는 것 확인.

---

## 2026-09-06 (12차) — `/edit`에서 곧바로 `/chart`로 가서 그 곡 드럼 채보를 편집하는 기능 신규

- **배경**: 예배 당일엔 여러 곡을 합친 통합 클릭 로직 프로젝트를 재생하며 진행하는데, 그 중 한 곡의 드럼 악보를 고쳐야 할 때마다 그 곡의 **개별** 로직 프로젝트 파일을 따로 열어야 `/chart`에서 편집할 수 있었던 게 번거롭다는 요청 — 로직을 아예 안 열어도 편집할 수 있게 함.
- **`/edit`**: 곡별 화면의 드럼 채보 업로드 영역에 "드럼 악보 수정" 버튼 신설(`WebServer.swift`) — 누르면 `/chart?song=곡이름`을 새 탭으로 엶(`/edit` 화면은 그대로 유지).
- **`/chart`**: `?song=` 쿼리로 열리면(`initLinkedSong()`, `chart.html`) 그 곡의 서버 저장분(`DrumChartStore`, `/api/drumChart`)을 자동으로 불러와 편집 상태로 만듦(파일 열기와 같은 원칙으로 지금 곡 목록엔 새 슬롯으로 들어가고 기존 작업은 안 건드림). 아직 업로드된 채보가 없으면 빈 마디로 시작해서 바로 만들 수 있게 안내.
  - 새 버튼 **"서버에 저장"**(곡 연결로 열렸을 때만 보임) — 누르면 `/save-drum-chart`로 곧장 그 곡에 반영. 기존 "저장"(파일로 내려받기) 버튼은 그대로 둠 — 사용자가 직접 관리하는 악보 폴더에도 따로 보관해야 해서 파일 다운로드도 계속 필요하다고 확인함.
- **⚠️ 구현 중 발견한 라우팅 버그**: `/chart` 경로가 `case ("GET", "/chart")`처럼 **완전 일치**로만 매칭되고 있었음 — `?song=...` 같은 쿼리스트링이 붙으면 문자열이 달라져 이 라우트에 안 걸리고 조용히 랜딩 페이지로 새고 있었음(이번에 기능을 만들다가 직접 발견 — 기존엔 `/chart`를 항상 쿼리 없이만 열어서 아무도 겪은 적이 없던 버그). `/api/drumChart`가 이미 쓰던 방식(`hasPrefix`)과 같은 패턴으로 `/chart?`도 별도 라우트로 추가해 수정.
- 로컬에서 전체 왕복 확인: 서버에 테스트 채보 저장 → `/chart?song=...`로 열어 제목·BPM·마디 내용이 정확히 반영되는 것 확인 → 마디 수정 후 "서버에 저장" → 다시 조회해서 수정 내용이 실제로 반영된 것까지 확인. 기존 "저장"(파일 다운로드) 버튼도 안 건드렸는지 확인.

---

## 2026-09-06 (11차) — 앱을 재시작하면 프로젝트 전환 감지가 아예 안 되던 버그 수정

- **증상 리포트**: "로직 프로젝트 파일이 바뀌었는데도 그걸 감지하지 못하고 있다."
- **원인**: 프로젝트 전환 감지(`checkProjectSwitch`)의 기준값(`scannedProjectTitle` — 스캔 당시 열려 있던 프로젝트명)이 `LogicPoller` 안 **메모리 변수에만** 있었음. 스캔 데이터 자체(마커·템포 등)는 디스크(`schedule_v2.json`)에 저장돼서 앱을 재시작해도 자동으로 복원되는데, 이 기준값만 저장이 안 돼서 앱을 껐다 켤 때마다 사라졌음 — 그래서 이번 세션에서 "스캔"을 한 번도 새로 안 누르면(디스크에 있던 예전 스캔 데이터를 그대로 쓰는, 아주 흔한 경우) 프로젝트를 실제로 바꿔도 비교할 기준 자체가 없어 감지가 전혀 동작하지 않았음.
- **수정**: `ScannedSchedule`(디스크에 저장되는 구조체)에 `projectTitle` 필드 추가, 스캔 완료 시 같이 저장. 앱 시작 시 디스크에서 스캔 데이터를 복원하는 지점(`AppDelegate`)에서 `logicPoller.scannedProjectTitle`도 같이 복원하도록 함(그러려면 `LogicPoller.scannedProjectTitle`을 `private`에서 외부 대입 가능하게 변경). 옛 저장 파일에는 이 필드가 없어도 Optional 기본값(`nil`)으로 안전하게 디코드되고, 다음 스캔부터 채워짐(마이그레이션 불필요).
- 참고: 어제(9/6) 앱을 오래 켜두고 다시 쓰려니 멈춘 느낌이라는 리포트도 같은 세션에서 나왔었는데, 그건 서버·프로세스 자체는 정상이었고 "손쉬운 사용 권한을 이번 실행에서 재허용 안 함"이 유력 원인이었음(빌드마다 초기화되는 별개의 알려진 현상) — 이번 버그와는 다른 문제.

---

## 2026-09-06 (10차) — 녹음이 선택한 마디가 아니라 엉뚱한 데 찍히던 버그 수정 (원래 있던 미완성 로직)

- **증상 리포트**: 녹음 버튼으로 특정 마디부터 녹음을 시작해도 노트가 그 마디가 아니라 이상한 곳에 찍힘.
- **원인**: `padHit()`의 실시간 녹음 위치 계산이 `tick()`이 이미 정확히 관리하고 있는 현재 마디(`curM`)·그 마디가 시작된 실제 시각(`mStartT`)을 **아예 안 쓰고**, `(지금 시각 − playStart) % 곡 전체 길이`를 **항상 곡의 맨 처음(마디 0)부터** 다시 훑어서 위치를 찾고 있었음 — "재생은 항상 1마디부터 시작한다"는 (지금은 더 이상 성립하지 않는) 전제로 짜여 있던 것. 구간을 선택하거나 리드인으로 그보다 앞 마디부터 재생을 시작하면, 실제 재생 위치와 이 계산이 완전히 어긋나 버림. 지난번(2026-09-06 5차) 녹음 버튼을 처음 노출시키면서 실사용이 가능해지고 나서야 이 기존 버그가 드러남 — 버튼이 없던 동안은 아무도 이 경로를 실행해본 적이 없었던 것.
- **수정**: 독자적으로 위치를 재계산하던 부분을 전부 지우고, `tick()`이 이미 갖고 있는 `curM`/`mStartT`를 그대로 가져다 씀(`bp = (지금 시각 − mStartT) / 그 마디 박 길이`) — 정확한 값이 이미 있는데 따로 구할 이유가 없었음. 마디 경계에서 값이 살짝 튀는 것에 대비해 `b`(박 인덱스)를 그 마디의 박 수 안으로 clamp하는 안전장치도 추가.
- 로컬에서 6마디짜리 곡의 5번째 마디(인덱스 4)를 선택해 녹음 버튼을 누르고, `curM`/`mStartT`를 그 마디가 막 시작한 상태로 맞춘 뒤 패드를 눌러 **정확히 5번째 마디에만** 노트가 찍히고 다른 마디(특히 0번째)엔 안 찍히는 것까지 확인.

---

## 2026-09-06 (9차) — BPM 입력창에서 1·2가 안 써지던 버그 수정

- **원인**: 방금 만든 BPM 직접 입력은 `<b contenteditable>`인데, 리얼드럼 탭의 커서 이동 단축키(1·2 = 좌우 화살표 대신)를 처리하는 전역 `keydown` 리스너 3곳이 전부 `e.target.tagName==='INPUT'`만 보고 있었음 — `<input>` 태그가 아닌 `contenteditable` 요소는 이 검사를 통과 못 해서, BPM 필드에 포커스가 있어도 1·2를 누르면 커서 이동으로 가로채지고 실제 숫자는 안 써졌음.
- **수정**: 세 곳(재생 단축키 Shift+Space, 실행취소 Cmd 조합, 커서 이동 1·2/화살표) 모두 `e.target.isContentEditable`도 같이 확인하도록 수정.
- 로컬에서 실제로 BPM 필드에 포커스 준 채 keydown 이벤트(key:'1', key:'2')를 그 요소에 직접 디스패치해서 `defaultPrevented`가 더 이상 안 켜지는 것까지 확인.

---

## 2026-09-06 (8차) — 리얼드럼 패드 단축키 직접 설정 + BPM 직접 입력 신설

- **리얼드럼 패드 단축키 커스터마이즈**: 패드를 **꾹 누르면**(550ms, 짧게 누르면 기존처럼 그 자리에서 바로 타격) 단축키 설정 팝업이 뜬다. 팝업이 열린 동안 키보드에서 누르는 키가 하나씩 토글로 추가/제거되고(같은 키 다시 누르면 빠짐), 크래쉬처럼 **여러 키**도 그대로 지원(예: E와 I 둘 다). Backspace로 마지막 키 제거, Enter로 완료, Esc로 취소, "모두 지우기"로 그 악기의 단축키를 아예 없앨 수도 있음(빈 채로 둬도 됨 — 터치로만 치면 됨).
  - `KEY_TO_PAD`(키→악기 매핑)를 새 객체로 안 바꾸고 **그 자리에서 delete/대입**하는 방식으로 고침 — 이 상수를 캡처해 둔 다른 클로저(실제 키 입력 핸들러 등)를 전부 다시 찾아 고칠 필요가 없어서 안전.
  - 저장한 키가 다른 악기에 이미 쓰이고 있으면 그쪽에서 자동으로 뺏어옴(한 키 = 한 악기, 충돌 방지).
  - `localStorage`(`drumchart.keymap`)에 저장돼 다음에 앱을 열어도 유지됨.
- **BPM 직접 입력**: BPM 숫자(`<b id="bpmVal">`)를 탭하면 그 자리에서 바로 타이핑해 바꿀 수 있음(`contenteditable`) — 슬라이더 대신 정확한 숫자를 바로 치고 싶을 때. 태그를 `<input>`으로 안 바꾸고 `<b>` 그대로 둔 이유: 이 태그를 겨냥한 CSS 선택자가 여러 군데(아이패드 레이아웃·재생 중 확대 등)에 있어서, 바꾸면 전부 안 먹음. 숫자 아닌 입력은 `beforeinput`에서 막고, 포커스를 벗어나면(blur) `bpmRange`의 min/max로 clamp 후 반영.
- 로컬 브라우저에서 직접 검증: 스네어에 새 키 2개 추가 후 저장 → 실제 키다운으로 스네어가 쳐지는 것까지 확인, 배지 텍스트·localStorage 저장 확인, 리드 악기 키 전부 지우면 배지가 사라지는 것 확인, BPM 필드에 큰 값·작은 값·잘못된 값 입력 시 각각 상한/하한/원래값으로 정상 처리되는 것까지 확인.

---

## 2026-09-06 (7차) — "구간" + "반복재생"을 같이 켜도 구간 끝에서 안 돌아오던 버그 수정

- **증상 리포트**: 구간을 지정하고 반복재생을 켠 뒤 그냥 재생(연습 모드 아님)을 누르면 반복이 안 됨.
- **원인**: `tick()`이 "지금 재생 범위의 끝에 도달했는지" 판정할 때 `const last = practiceRun ? rangeEnd : measures.length-1;`로, **연습 모드일 때만** `rangeEnd`(구간 끝)를 보고 있었음. 일반 재생은 `loopRange`로 구간을 정해도 `rangeEnd`가 이미 맞게 채워지는데(`startPlay`), 끝 판정 자체가 그 값을 무시하고 항상 "곡 전체의 마지막 마디"를 봐서 구간을 벗어나 곡 끝까지 계속 흘러가 버렸음 — 언젠가 진짜 곡 끝에 도달하면 그제서야 반복(rangeStart로 복귀)되니 "반복이 아예 안 된다"로 보였음.
- **수정**: `const last = rangeEnd;`로 통일. `startPlay`의 세 분기(연습·구간 지정·일반) 전부 이미 `rangeEnd`를 상황에 맞게 채워두고 있어서(구간 미지정 시엔 `rangeEnd=measures.length-1`로 예전과 동일) `practiceRun` 분기 자체가 필요 없었음.
- 로컬에서 10마디 만들고 `loopRange={a:2,b:4}` 지정 후 확인 — `rangeEnd`가 4로 정확히 반영되는 것까지 확인(수정 전엔 이 경우도 `last`가 9였음).

---

## 2026-09-06 (6차) — 녹음 버튼을 독(재생 버튼 옆)으로 이동 + 원클릭으로 단순화

- 방금 만든 "녹음" 버튼이 "켜고 → 재생을 따로 누르는" 2단계라 불편하다는 피드백 — **누르면 그 자리에서 바로** 한 마디 리드인+녹음이 시작되도록 변경(`startPlay({recordLeadIn:true})`를 클릭 핸들러에서 직접 호출). 이미 재생 중이면 정지로 동작(재생 버튼과 동일 의미).
- 버튼 위치를 리얼드럼 탭 안 사이드 패널에서 **독(dock)의 재생 버튼 바로 옆**으로 이동, 빨간 원 아이콘으로 변경. 여전히 리얼드럼 탭에서만 보임(`setMode()`가 `display` 토글) — 그리드/연습 탭에서는 자동으로 숨김.
- 리드인 카운트 중과 실제 기록 중을 구분할 수 있게, `recording`이 실제로 `true`가 되는 순간(`tick()`의 `recordArmAt` 매치 지점)에만 버튼이 꽉 찬 빨강+펄스(`.live`)로 바뀌도록 `syncRecordBtn()` 추가.
- 로컬에서 실제 클릭 한 번으로 `playing`·`recording`이 동시에 `true`가 되는 것까지 확인.

---

## 2026-09-06 (5차) — 리얼드럼 탭 전용 "녹음" 버튼 신설 + 메트로놈 8비트 서브클릭 신설

- **녹음 버튼**: 지난 대화에서 "실시간 녹음 로직은 있는데 켤 스위치가 없다"고 확인했던 것 — 리얼드럼 탭 안(`#drumMode`)에만 "녹음" 토글 버튼을 추가해서, 켠 채로 재생 버튼을 누르면 `startPlay({recordLeadIn:true})`가 호출되도록 연결. HTML 구조상 `#drumMode`의 자식이라 그리드 탭에서는 자동으로 안 보임(요청대로).
  - **덤으로 발견한 버그도 같이 수정**: 선택한 마디가 1마디(맨 처음)일 때는 리드인 갈 "그 앞 마디"가 없어서 `recording`이 영영 `true`가 안 되는 문제가 있었음(리드인 조건 `rangeStart > 0`이 막혀 else로 빠지는데, 그 else가 `recording`을 아예 안 건드림) — 1마디부터 녹음을 못 하는 상태였음. `rangeStart===0`이면 리드인 없이 즉시 `recording=true`로 시작하도록 수정. 구간 반복(`loopRange`) 걸린 상태에서 녹음하는 경로에도 동일하게 적용(원래 `recordLeadIn` 자체를 아예 안 보고 있었음).
- **메트로놈 8비트 서브클릭**: 기존엔 박(4분음표)마다 한 번만 딸깍였음 — `metBtn`을 **꾹 누르면**(짧게 탭하는 기존 3단 순환과 별개) 박 사이 "&"에도 낮은 음+작은 소리로 클릭이 들리는 8비트 모드가 켜짐. `SOUND.click(t, weak)`에 `weak` 파라미터 추가(약박은 피치·볼륨 낮춤), `tick()`의 클릭 스케줄링을 박 단위/반박 단위로 유연하게 바꿈(`lastClickBeat`를 두 모드에서 같은 변수로 재사용, 단위만 다르게 셈). 버튼에 8비트 켜짐을 알려주는 작은 배지(왼쪽 위, 기존 예비박 배지는 오른쪽 아래라 안 겹침) 추가.
- 로컬 브라우저에서 JS로 직접: 녹음 버튼 리얼드럼 탭 전용 노출 확인, 1마디부터 녹음 시작 시 `recording=true` 즉시 확인, 2마디 이상에서 리드인 정상 동작(`recordArmAt`) 확인, `metSubOn()`·약박 클릭 사운드 호출까지 에러 없이 확인.

---

## 2026-09-06 (4차) — `/chart`에 32비트(32분음표) 그리드 입력 신규 지원

- **배경**: 그리드의 박 나누기(`DIV_UI`)가 지금까지 8분/셋잇단/16분/16분셋잇단까지만 있었음 — 32분(`div=8`)은 `DIV_OK`(데이터 모델)엔 이미 있었지만 UI에서 빠져 있었는데, 이유가 "32분쉼표를 그릴 폰트 글리프가 없어서"였음(코드 주석으로 확인). 사용자가 32비트 입력이 필요하다고 해서 이번에 뚫음.
- **32분쉼표 글리프 신규 제작** (`REST_GLYPH.e32`, `chart.html`): 이 앱의 쉼표(4분·8분·16분)는 전부 Bravura(SIL OFL) 폰트에서 뽑은 정식 아웃라인을 쓰고 있어서, 32분쉼표도 손으로 새로 그린 근사 도형을 넣으면 이질감이 클 것 — 대신 **16분쉼표(`e16`) 글리프 자체의 아래쪽 고리(loop)를 그대로 한 번 더 평행이동해서 셋째 고리를 끼워 넣는 방식**으로 만듦(실제 존재하는 폰트 곡선을 재사용, 되돌아가는 가는 줄기 부분만 새 길이에 맞춰 단순화). 로컬에서 렌더링해 16분쉼표와 나란히 비교 스크린샷으로 확인 — 고리 하나 더 얹은 자연스러운 모양으로 잘 나옴.
- `REST_BY_FLAGS`에 `'e32'` 추가, `drawRest`의 flags 상한을 2→3으로, `DIV_UI`에 `8` 추가. 나머지(빔 단수 계산·박 세는 말 `COUNT_WORDS[8]`·`DIV_NAME[8]='32분'`)는 이미 일반화돼 있거나 미리 준비돼 있어서 손댈 필요 없었음.
- 로컬 브라우저에서 실제로 한 박을 32비트로 나누고 음표 사이에 32분쉼표가 뜨는 경우(온셋 앞 한 칸만 비움)까지 만들어서 확인 완료.

---

## 2026-09-06 (3차) — 플램이 리얼드럼(패드 탭) 입력에서는 전혀 반영 안 되던 버그 수정

- **원인**: 플램 토글(`flamMode`)을 그리드 탭의 셀 클릭 핸들러에만 연결해뒀고, "리얼드럼" 탭(드럼 패드를 손가락/키보드로 두드려 입력하는 방식, `padHit()`)은 완전히 별개 코드 경로라 `flamMode`를 아예 확인하지 않았음 — 사용자가 리얼드럼 탭에서 플램 켜고 패드를 눌러도 악보에 아무 변화가 없던 게 이 때문(실사용 스크린샷으로 확인). 그리드 탭에서만 되고 리얼드럼 탭에서는 조용히 무시되니 "버튼이 눌렸는데도 안 된다"로 보였음.
- **수정** (`padHit()`, `chart.html`): 정지 상태에서 커서로 스텝 입력할 때 `flamMode`면 그리드와 완전히 동일한 규칙(`toggleFlam`)으로 처리 — 빈 칸이면 음+플램을 같이 찍고, 이미 음이 있으면 플램만 토글(음은 안 지워짐. 원래 리얼드럼의 "같은 표기 다시 누르면 음이 지워짐" 규칙과는 다르게 동작해야 해서 별도 분기). 재생 중 실시간 녹음 경로에도 동일하게 `flamMode`면 플램 플래그를 같이 기록하도록 추가.
- 플램 버튼의 안내 문구(title)도 예전 "이미 찍힌 음에만 붙는다"는 문구가 그대로 남아있어서 수정(2026-09-06 2차에서 동작은 고쳤는데 문구를 안 고쳤던 것).
- 로컬 브라우저에서 리얼드럼 모드(`inputMode='drum'`)로 전환 후 `flamMode` 켠 채 `padHit('snare')` 직접 호출해 음+플램이 한 번에 찍히는 것 확인 완료.

---

## 2026-09-06 (2차) — 플램 빈 칸 버그 수정 + 마디 여러 개 선택 비우기·삭제 버그 수정 + "곡 목록" 버튼 재노출

- **플램: 빈 칸에서 안 되던 버그 수정** (`chart.html` `toggleFlam`): 처음엔 "이미 음이 있는 칸에만 플램이 붙는다"로 만들었는데, 실사용에서 "플램 켜고 빈 칸을 찍어도 아무 반응이 없다"고 리포트됨 — 자연스러운 사용 순서(플램 모드부터 켜고 찍는 것)를 막고 있었던 것. 이제 플램 모드에서 빈 칸을 찍으면 지금 선택된 표기(art)로 음을 새로 찍으면서 플램도 같이 켜짐. 이미 음이 있는 칸은 기존대로 플램 유무만 토글.
- **"비우기"·"삭제" 버튼이 여러 마디를 선택해도 1마디만 처리하던 버그 수정** (`chart.html`): 둘 다 `measures[selMeasure]`(기준점 1마디)만 보고 있었음 — 실제 선택 범위(`selA()`~`selB()`)를 반영하도록 수정. "삭제"로 선택 범위가 곡 전체면(마지막 한 마디까지 남기지 않고 전부 지우면) 빈 마디 1개만 남겨서 곡이 통째로 없어지지 않게 함.
- **"곡 목록"(라이브러리) 버튼 상시 노출**: 이 버튼(과 그 안의 "+ 새 곡")으로 가는 유일한 길이 앱을 열 때 딱 한 번 뜨는 "지난 작업을 이어서 열었습니다" 토스트뿐이었음(2026-08-13에 버튼 자체를 `display:none`으로 숨기고 이 토스트에만 의존하게 바꿨던 것) — 그 토스트를 놓치면 다른 곡 채보를 새로 시작할 방법이 없어서 "새로 채보하려는데 이전 작업이 계속 자동으로 열린다"는 리포트로 발견. 버튼을 다시 항상 보이게 해서 "곡 목록 → + 새 곡"으로 언제든 빈 마디 1개부터 새로 시작 가능(기존 작업은 지워지지 않고 곡 목록에 남아 나중에 다시 열 수 있음 — 완전 삭제가 아니라 새 슬롯을 만드는 방식이라 이 편이 파괴적인 "전체 지우기"보다 안전).
- 세 가지 모두 로컬 브라우저(`localhost:8888/chart`)에서 JS로 직접 재현·검증 완료(여러 마디 선택 후 비우기/삭제 결과 배열 확인, 빈 칸 플램 토글 결과 확인, 곡 목록 팝업에서 "+ 새 곡" 버튼 노출 확인).

---

## 2026-09-06 — 재생 위치 왔다갔다하는 버그 2건 수정 + `/chart`에 플램(장식음) 표기 신설

- **`barPosition` 지터 가드 누락 수정** (`StateEngine.swift`): `/drum`·`/chart` 라이브 하이라이트가 쓰는 `state.barPosition`(실제 절대 마디 위치)이, 같은 파일의 다른 위치값들(`currentBarFloat`, `sectionElapsedSec`)과 달리 "재생 중엔 뒤로 안 가게" 막는 단방향 가드가 빠져 있었음. MTC 원시 시간값이 재생 중 아주 살짝(ms 단위) 뒤로 튀는 지터가 있는데, 이게 그대로 다음 마디로 넘어가는 경계 바로 앞에서 마디 인덱스가 한 번 뒤로 갔다가 다시 앞으로 오는 걸로 보였음. `lastSentBarPosition` 추가해 같은 원리의 가드 적용, 실제 점프(위치 이동) 시에는 리셋해서 그대로 통과.
- **`/edit`(가사 편집 화면) 재생헤드가 왔다갔다하던 버그 수정** (`WebServer.swift` 임베디드 JS `liveAbsSec`): `/drum`·`/chart`와 달리 `/edit`의 재생헤드는 서버에서 받은 마지막 값에 `Date.now()` 경과분을 더해 **클라이언트에서 직접 보간**하는 방식이었는데, 메시지 도착 간격이 네트워크 지연으로 들쭉날쭉하면 이 보간값이 다음 실제 서버값을 잠깐 앞질렀다가 그 값이 도착하는 순간 되돌아가길 반복 — 재생 중인데도 화면이 흔들림. `liveGuard`로 같은 원리의 단방향 가드 추가(0.3초 미만 되돌아감만 지터로 간주해 막고, 그 이상은 실제 되감기로 보고 통과).
- **`checkSectionEnd()`가 정지 중에도 실행되던 버그 수정** (`StateEngine.swift`) — **가장 심각했던 버그, 실측 로그로 확인**: 정지 상태에서도 `checkSectionEnd()`가 그대로 호출되는데, 이 함수는 **재생 중에만 갱신되는** `mtcTime`(마지막으로 재생했던 옛 시간, 정지 중엔 그 값에 멈춰 있음)을 기준으로 "지금 섹션이 벌써 끝났는지" 판정함. 정지 위치가 그 옛 mtcTime보다 앞쪽 섹션이면 "벌써 끝났다"고 오판해 다음 섹션으로 넘기고, 바로 다음 폴링에서 `transportBar` 기반 판정이 원래 섹션으로 되돌리고, 되돌아간 순간 다시 같은 오판이 반복 — **두 섹션 사이를 초 단위로 무한 왔다갔다**(`~/Library/Logs/Indicator/indicator_debug.txt`에서 실측: `transportBar=1` 고정인데 currentSection이 계속 바뀜). `checkSectionEnd()` 맨 앞에 `guard mtcIsPlaying else { return }` 추가 — 정지 중엔 아예 실행 안 함(정지 중 섹션 판정은 이미 `transportBar` 기반 로직이 전담하므로 이 함수가 필요 없음).
- **`/chart`에 플램(flam) 표기 신설**: 실제 드럼 채보 영상을 옮기던 중 발견 — 스네어 등에 붙는 장식음(작은 임음표+슬래시)이 시스템에 아예 없었음. 표기(art: 기본/고스트/액센트/오픈)와 성격이 달라서(어떤 art의 음에든 겹쳐 붙을 수 있는 독립 장식이라) 배타적 선택 대신 **별도 토글 모드**로 구현.
  - 데이터: 악기별 `<id>Flam` boolean 배열 신설(`emptyMeasure`/`normalizeMeasures`에서 본 배열과 완전히 같은 방식으로 생성·정규화). 음 자체가 꺼지면(`setCell`·`clearAtCursor`·`clearMeasure`·패턴 브러시) 플램도 항상 같이 지워짐 — 음 없는 플램은 존재할 수 없게 강제.
  - `setDiv`(박 나누기 변경)에서 본 배열과 플램 배열을 **완전히 같은 스플라이스 로직**으로 같이 옮기도록 함 — 따로 계산하면 인덱스가 어긋나는 버그가 나기 쉬움.
  - UI: 표기 팔레트 옆 "플램" 토글 버튼 — 켜져 있는 동안 그리드를 눌러도 음(art)은 안 바뀌고, **이미 음이 있는 칸에만** 플램 유무가 토글됨(빈 칸 누르면 토스트로 안내). 그리드 칸 왼쪽 위 작은 삼각 마크로 표시.
  - 렌더링: 주 음표 바로 앞·왼쪽 위에 작은 임음표(꼬리 하나)를 그리는 `drawFlam()` 신설, 표준 타악보 플램 표기와 같은 자리.
  - 로컬 브라우저(`http://localhost:8888/chart`)에서 실제로 스네어 노트에 플램 켜기/끄기, SVG 좌표(주 음표 대비 -10x/+2.5y, 작은 노트헤드 rx 3.1 vs 본 노트헤드 rx 5.4)와 `measures[i].snareFlam` 배열 값까지 직접 확인해 정상 동작 검증 완료.

---

## 2026-09-03 (4차) — 앱 전역에서 "Mai Drum"(포크 원본) 흔적 제거

- **배경**: `/chart`는 친구가 만든 "Mai Drum"을 포크해서 시작한 화면인데, 사용자가 "여러 군데에 mai라는 단어가 보인다"고 신고 — 앱 전체를 grep해서 남아있는 흔적을 전부 정리.
- **파일 확장자 `.mai.json` → `.drum.json`**: `/chart`의 "저장" 버튼이 만드는 파일명, 관련 안내·오류 메시지(`/chart` 자체·`/edit` 업로드 버튼·`/drum` 빈 상태 안내) 전부 새 확장자로 변경. **하위 호환은 그대로 유지** — 파일을 여는 로직(`looksLikeScore`)은 애초에 파일 내용(JSON 구조)만 보고 확장자는 안 보므로 기존 `.mai.json` 파일도 계속 정상적으로 열림. `/edit`의 업로드 파일선택창 `accept` 속성에는 `.mai.json`과 `.drum.json`을 둘 다 남겨서, 예전에 내보내둔 파일도 파일 선택 대화상자에서 계속 보이도록 함. 파일에 제목이 없을 때 파일명에서 확장자를 떼는 정규식도 신·구 확장자 둘 다 처리하도록 수정.
- **브라우저 localStorage 키 네임스페이스 `maidrum.*` → `drumchart.*`**: `/chart`가 곡 라이브러리(색인·곡 본문)·악보 높이·Studio 모드 같은 걸 저장하던 키 이름에도 원본 포크의 흔적이 그대로 남아있었음(개발자 도구로만 보이는, 사용자에게 직접 안 보이는 부분이지만 "전역에서" 지워달라는 요청 범위에 포함해서 함께 정리). **기존에 이미 저장돼 있던 곡 데이터가 사라지면 안 되므로**, 앱 시작 시 새 키가 비어있고 옛 `maidrum.*` 키에 데이터가 있으면 한 번만 새 이름으로 안전하게 복사(쓰기 성공을 읽어서 재확인한 뒤에만 옛 키 삭제 — 기존 `libMigrate`가 쓰던 것과 동일한 원칙)하는 `namespaceMigrate()`를 신설해 `libLoad()` 전에 실행. 옛 단일 초안 키(`maidrum.draft`)와 악보 높이 설정(`maidrum.scoreH`)도 새 이름이 비어있을 때만 옛 이름을 폴백으로 읽도록 처리.
- **footer 브랜딩**: "MAI SERIES · MAISTRO" 문구를 "드럼 스코어"로 교체(이미 이전 세션에 대부분 정리됐던 브랜딩 잔재 중 마지막으로 남아있던 곳).
- CLAUDE.md 등 과거 작업 로그(이 파일 자체, "Mai Drum을 포크해서 시작했다"는 배경 설명 등)에 남아있는 "mai" 언급은 **의도적으로 그대로 둠** — 실제 작업 이력을 설명하는 기록이라 지우면 오히려 맥락을 잃음. 코드 안에 남은 `maidrum.*`도 전부 위 하위 호환 목적의 주석·변수명이라 의도적으로 남긴 것.

---

## 2026-09-03 (3차) — 크레센도/디크레센도 위치를 오선 아래 → 위로 변경

- 신설 직후 사용자 요청으로 즉시 조정: 하이핀을 오선 **아래**(`top + 4*LG + 30`)에서 **위**(`top - 22`)로 이동. `notation.js`·`chart.html` 공유 헬퍼(`drawDynamicsRow`) 한 곳만 고치면 `/chart`·`/drum`·인쇄 전부 반영됨(3곳에 나눠 그리던 이전 기능들과 달리, 크레센도는 처음부터 각 파일에 헬퍼 함수 하나씩만 있어서 수정 지점이 2곳뿐이었음).
- 마디 번호·자유 텍스트가 이미 차지한 y존(top-14~top-10) 위쪽, TOP_PAD로 이미 확보돼 있던 여백 안에 들어가서 SYS_H를 더 늘릴 필요 없이 그대로 들어감. 라이브로 좌표(하이핀 y=오선 시작선 y보다 22px 위)와 실제 스크린샷까지 확인.
...
- 상단 헤더(제목·상태 표시줄) 전체 삭제 — 공연 중엔 그 자리도 악보로 쓰는 게 낫다는 판단.
- **구조 전면 재설계**: 예전엔 곡이 바뀔 때마다(승격 포함) `renderLines()`가 `linesEl.innerHTML`을 통째로 교체해서, 데이터는 이미 준비돼 있어도(다음 곡 미리보기 승격 케이스) 화면이 "전환"되는 느낌이 있었음. 이제 `renderedSongs`(현재+다음 곡을 DOM에 순서대로 계속 쌓아두는 배열)로 바꾸고, **실제로 곡이 전환되는 순간엔 다시 그리지 않고 "지금 재생 중" 포인터(배열 인덱스)만 옮긴다** — DOM 엘리먼트(svg 등)를 그대로 재사용. 곡 구분선도 "다음 곡 · 곡명"에서 그냥 "곡명"으로 바꿔 계속 이어지는 타임라인처럼 보이게 함(승격돼도 문구를 안 바꿔도 되게).
- 라이브 검증: 승격 전후 같은 svg DOM 엘리먼트 참조(`===`)가 유지되는 것까지 확인(재생성 없음 확정).
- 곡마다 고유 DOM id 접두어(`song{N}_l{gi}`)를 부여해 여러 곡이 동시에 DOM에 떠 있어도 id 충돌 없음.

### 마디 위 텍스트를 섹션명과 같은 크기로 확대
- 10px → **16px/700**(섹션명과 동일)로 확대 — 공연 중 잘 안 보인다는 피드백. `notation.js`·`chart.html`(편집·인쇄) 3곳 전부.

### 크레센도/디크레센도 하이핀 기능 신설
- 마디(구간) 선택 후 "크레센도"/"디크레센도" 버튼 — 연속으로 같은 태그가 붙은 마디들에 걸쳐 오선 아래에 하이핀(`<`/`>` 모양) **하나로 크게** 이어서 그림(`measures[i].dynamic = 'cresc'|'decresc'`, 연속 태그를 자동으로 합쳐 그리는 `drawDynamicsRow()`). 이미 선택 구간 전체가 같은 종류면 토글 해제.
- `notation.js`·`chart.html`(편집·인쇄) 3곳 모두 적용, `normalizeMeasures()`에 보존 추가. 위치 계산용 캐시가 없어도(`o.cache` 미전달 호출부) 항상 로컬 캐시를 만들어 쓰도록 해서 하이핀이 어디서든 그려지게 함.
- 라이브 검증: 3마디 선택 → 크레센도(하이핀 좌→우 벌어짐) → 토글 해제 → 디크레센도(반대 방향) → 저장/불러오기 왕복까지 전부 확인.

---

## 2026-09-02 (4차) — 마디 위 텍스트 위치 왼쪽으로, `/drum` 가사를 마디 아래로 이동

- **마디 위 자유 텍스트**(바로 앞 세션에서 신설): 우측 정렬 → **좌측 정렬**로 변경, 마디 번호(x=mx+2) 바로 오른쪽(x=mx+16)에 이어붙여 서로 안 겹치게. `notation.js`·`chart.html`(편집·인쇄) 3곳 전부 동일 적용.
- **`/drum` 섹션명·가사 캡션**: 마디 **위**(오선 시작 전 56px 여백에 절대좌표) → **아래**로 이동. `position:absolute;top:0` 방식은 svg 렌더 높이가 반응형(뷰포트 폭에 따라 변함)이라 CSS만으로 "그 아래"를 정확히 계산할 수 없어서, 마크업 순서를 `svg`→`.cap`로 바꿔 일반 문서 흐름에 맡기고 가로 위치(그 섹션이 실제 시작하는 마디)만 `margin-left: N%`로 유지하는 방식으로 재구현(기존 `left: N%` position:absolute 대신). svg 위쪽 여백도 캡션 공간 확보용 56px → 원래 6px로 되돌림(더 이상 안 필요).
- fetch 모킹으로 텍스트 위치(마디 번호 옆)·가사 위치(오선 바로 아래, DOM 순서까지) 라이브 검증 완료.
