...
2. **섹션명·가사 캡션이 항상 정확히 한 마디 일찍 표시되던 버그**: `mi = Math.floor(o.barPosition - startBarPos)` — 마커가 정확히 마디 경계(예: 8.0)에 있어도 템포맵 보간 부동소수점 오차로 `7.999999...`처럼 살짝 못 미치게 나올 수 있어 `floor`가 한 마디 전으로 잘못 내림됨. `+1e-6` 보정 추가(캡션 계산·라이브 하이라이트 계산 두 곳 다 동일 적용). 라이브 하이라이트는 매 틱 재계산돼 순간만 지나면 자연 보정되지만, 캡션은 곡 로드 시 한 번만 계산해 고정하므로 오차가 그대로 굳어 눈에 띄었음.
