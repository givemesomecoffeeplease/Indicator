...
## 2026-09-02 (3차) — 드럼 마디 위 자유 텍스트(연주 큐) 입력 기능 신설

- 요청 전 계획부터 확인(AskUserQuestion): `/drum`(실제 공연 화면)에도 보일지 vs `/chart`에서만 보일지 → **둘 다**로 확정. 입력 방식은 마디 선택 후 버튼→프롬프트 창으로 확정.
- **데이터**: `measures[i].text`(자유 문자열) 신규 — `repeatPrev` 등과 동일하게 `normalizeMeasures()`에서 보존.
- **입력**: 마디 툴바에 "텍스트" 버튼 신규(`textM`) — 클릭 시 프롬프트로 현재 텍스트를 보여주고 수정, 빈 값으로 저장하면 삭제. 버튼 자체도 텍스트가 있으면 라벨이 `텍스트: 필인 준비`처럼 바뀌고 `on` 스타일이 켜짐(리핏 마디 버튼과 동일한 패턴).
- **표시 위치**: 마디 번호(좌상단 회색 숫자)와 같은 높이에서 **마디 우측 정렬**로 배치(보라색 `#7B4BE0`, 굵게) — 새로 세로 공간을 안 늘려도 되고, `/drum`의 기존 섹션명·가사 캡션(더 위쪽 별도 영역)과 절대 안 겹침.
- `notation.js`(`/drum` 공유 엔진)·`chart.html`의 편집 렌더 엔진(`paintAutoScore`)·인쇄/PDF 내보내기용 `paint()`까지 3곳 전부 동일하게 추가.
- 라이브 검증: 버튼 클릭(프롬프트 모킹) → 데이터 저장·버튼 라벨 갱신·악보에 실제 렌더링(우측정렬·색상) → `normalizeMeasures()` 왕복 후에도 유지 → `Notation.paint()`(=`/drum`이 쓰는 바로 그 함수)에서도 동일하게 렌더링되는 것까지 전부 확인.

---

## 2026-09-02 (2차) — `/drum` 다음 곡 악보 미리보기(스크롤 대기) 신설

### 요청
곡이 전환되는 순간 악보가 갈아끼워지는 방식이었는데, 그 전에 미리 다음 곡 악보를 스크롤해서 보고 싶다는 요청 — 지금 재생 중인 곡 악보 아래에 다음 곡 악보를 이어붙여 스크롤로 미리 볼 수 있게 해달라는 것.

### 구현
- 채보 로딩 로직을 `fetchChartData(songName)`(전역 상태를 안 건드리고 `{measures, startBarPos, lineGroups}`만 반환)로 분리, current/next 양쪽에서 재사용.
- `current*`와 완전히 대칭인 `next*` 상태(`nextSongName`/`nextMeasures`/`nextStartBarPos`/`nextLineGroups`) 신규 — `/api/sections`의 `st.songs[currentSongIndex+1]`로 다음 곡 이름을 얻어 조용히(화면 전환 없이) 미리 불러온다.
- `renderLines()`가 현재 곡 줄들 다음에 점선 구분선("다음 곡 · 곡명")과 다음 곡 줄들을 이어서 렌더링. 다음 곡은 아직 재생 중이 아니므로 하이라이트용 `posCache`와 완전히 별도인 `nextPosCache`(버리는 값)로 그려 하이라이트 로직과 절대 섞이지 않게 분리.
- **곡이 실제로 전환되는 순간**: 이미 미리 불러온 next가 바로 그 곡이면 재요청 없이 그대로 "승격"(next→current로 값만 옮김) — 사용자가 스크롤로 미리 보던 그 악보가 그대로 메인이 되고, 그 즉시 그다음 곡을 새로 미리 불러온다. next를 먼저 비우고 나서 `applyCurrentData`(→`renderLines`)를 호출해야 승격된 데이터가 "현재 곡"과 "다음 곡 미리보기" 두 군데에 겹쳐 그려지지 않음(순서 실수로 처음에 한 번 겪음, 고침).
- **버그 하나 발견·수정**: `curSongName`은 `loadSong()` 시작과 동시에(비동기 fetch 완료 전에) 곧장 목표 곡 이름으로 바뀌는 기존 설계라, `applyState()`에서 "로딩 아직 안 끝남"을 감지하는 용도로 `curSongName` 비교를 그대로 쓰면(원래 코드 구조상 매 틱 다시 계산되던 걸 이번에 함수로 분리하며 처음 실수) 항상 참이 되어 안 걸러짐 — 별도 `songDataReady` 플래그(데이터가 실제로 채워졌을 때만 true)를 추가해 해결.
- 라이브로 fetch를 모킹해 3곡 시나리오(A재생·B다음곡, A→B 실제 전환·C 다음곡으로 새로 준비, 하이라이트가 미리보기 줄로 안 새는지)까지 전부 검증 완료.

---

## 2026-09-02 — `/drum`·`/chart` SSE 재연결 로직 보강 (장시간 켜두면 악보가 안 보이는 문제)

### 증상
드럼 뷰어(`/drum`)를 오래 켜놓고 쓰면(악보만 보는 용도의 별도 맥 등) 어느 시점부터 화면이 갱신을 멈추고 그대로 정지해 보이는 문제가 보고됨.

### 원인 추정
실시간 갱신은 전부 SSE(`EventSource`, `/events`)로 동작하는데, 같은 코드베이스 안에서도 화면마다 재연결 처리 방식이 서로 달랐음:
- `index.html`(밴드뷰어)·`singer.html`(싱어뷰어): 연결이 끊기면 `es.close()` 후 지수 백오프(1초→2초→...→최대 10초)로 **직접 재연결**하는 로직이 이미 있었음.
- `drum.html`·`chart.html`: 이 로직이 없이 브라우저 `EventSource`의 **자동 재연결에만 의존**(`chart.html`은 "EventSource가 알아서 재연결을 시도한다"는 주석까지 있었음).
- 브라우저 EventSource는 보통 알아서 재연결하지만, 탭이 오래 백그라운드에 있거나 macOS/브라우저가 절전 상태로 들어가면 이 자동 재연결이 실패하는 사례가 실제로 있음(특히 Safari) — 그 경우 연결이 조용히 끊긴 채 화면은 아무 갱신도 못 받고 멈춰있게 되어 증상과 일치.

### 수정
`drum.html`·`chart.html`의 SSE 연결부를 `index.html`/`singer.html`과 동일한 명시적 재연결 패턴으로 교체 — `es.onerror` 시 `es.close()` 후 지수 백오프로 재시도. 콘솔 에러 없이 정상 연결 확인, 다만 실제 "오래 켜둔 뒤 절전에서 재연결" 시나리오 자체는 재현·검증이 어려워(수 시간~수 일 단위 재현이라) **근본 원인 확정은 아니고 유력한 가설에 기반한 방어적 수정** — 다음에 같은 증상이 재발하면 다른 원인(예: 서버 쪽 `SSEBroadcaster`의 죽은 연결 정리 타이밍, macOS 앱 자체의 절전 대응 부재)도 같이 점검할 것.

---

## 2026-08-30 — `/chart` 리얼드럼 입력 단위에 32비트 추가

- 저장 쪽(`DIV_OK = [2,3,4,6,8]`)은 원래부터 div=8(32분음표)까지 지원하고 있었음 — 4비트/8비트/16비트 UI 버튼만 있고 32비트 진입 방법이 없었을 뿐. `entryDiv32` 버튼(entryDiv=8) 신규 추가.
- 단축키: 기존 "8을 빠르게 두 번 → 16비트"를 "8을 연속으로 빠르게 누른 횟수(1/2/3번 이상)로 8→16→32비트 순환"으로 확장(`press8Streak` 카운터, 350ms 이내 연속 입력만 인정).
- 라이브 검증: 32비트 모드에서 실제로 노트를 찍으면 그 박의 div가 4→8로 무손실 확장(다른 박·발 성부는 안 건드림), 스텝 점도 4박×8칸=32개 정상 렌더링, 단축키 8·8·8 연속 입력 시 2→4→8 순서로 정확히 전환되는 것까지 확인.
